...
// Essais gratuits (free trial) — attribution temporaire d'un rôle
const TRIAL_CHANNEL_ID = process.env.TRIAL_CHANNEL_ID || '1535341313220944044'; // salon du tableau de suivi
const TRIAL_CHECK_INTERVAL_MS = 60 * 1000; // vérifie toutes les 60s si un essai est expiré
...
function loadState() {
  try {
    ...
    return {
      ...
      knownTikTokVideos: [],
      trials: {}, // { "userId-roleId": { userId, roleId, roleName, startedAt, endsAt, addedByTag } }
      trialMessageId: null,
    };
  }
}
...
if (!state.trials) state.trials = {}; // compat avec un ancien state.json existant
if (state.trialMessageId === undefined) state.trialMessageId = null;
...
async function postRoleReactionMessage(triggeredByTag = 'Automatique', triggeredByMention = '') {
  ...
  logAction('📥', 'Message rôles publié', `Par : ${triggeredByMention} (**${triggeredByTag}**)`, 0xE8B4D0);
  return { message: sent };
}
...
// ============================================================
// 3sexies. ESSAIS GRATUITS (FREE TRIAL) — rôle temporaire avec suivi
// ============================================================

// Construit l'embed du tableau de suivi (un seul message, mis à jour à chaque changement)
function buildTrialEmbed() {
  const entries = Object.values(state.trials).sort((a, b) => a.endsAt - b.endsAt);

  const embed = new EmbedBuilder()
    .setColor(0xE8B4D0)
    .setAuthor({ name: 'CEAXUR', iconURL: client.user?.displayAvatarURL() })
    .setTitle('🧪 Suivi des essais gratuits')
    .setTimestamp();

  if (entries.length === 0) {
    embed.setDescription("Aucun essai gratuit en cours actuellement.\n\nLance-en un avec `!trial @membre @rôle 30j`.");
  } else {
    const lines = entries.map((t) => {
      const endsSec = Math.floor(t.endsAt / 1000);
      return (
        `👤 <@${t.userId}> — 🎭 **${t.roleName}**\n` +
        `⏳ Expire <t:${endsSec}:R> _(le <t:${endsSec}:f>)_`
      );
    });
    embed
      .setDescription(lines.join('\n\n'))
      .setFooter({ text: `${entries.length} essai(s) actif(s)`, iconURL: client.user?.displayAvatarURL() });
  }

  return embed;
}

// Poste ou édite le message du tableau. Si l'ID mémorisé est perdu (redéploiement),
// on retrouve le dernier message du bot dans le salon plutôt que d'en recréer un nouveau.
async function updateTrialEmbed() {
  if (!TRIAL_CHANNEL_ID) return;
  const channel = client.channels.cache.get(TRIAL_CHANNEL_ID);
  if (!channel) return console.error('❌ TRIAL_CHANNEL_ID invalide');

  const embed = buildTrialEmbed();

  try {
    if (state.trialMessageId) {
      const msg = await channel.messages.fetch(state.trialMessageId).catch(() => null);
      if (msg) {
    ...
    const ownMessage = recent?.find((m) => m.author.id === client.user.id);
    if (ownMessage) {
      await ownMessage.edit({ embeds: [embed] });
      state.trialMessageId = ownMessage.id;
      saveState(state);
      return;
    }

    const sent = await channel.send({ embeds: [embed] });
    state.trialMessageId = sent.id;
    saveState(state);
  } catch (err) {
    console.error('❌ Erreur mise à jour tableau essais :', err);
  }
}

// Vérifie les essais expirés, retire le rôle et met à jour le tableau si besoin
async function checkTrials() {
  ...
  for (const [key, trial] of Object.entries(state.trials)) {
    ...
    try {
      const guild = client.guilds.cache.get(GUILD_ID);
      const member = guild ? await guild.members.fetch(trial.userId).catch(() => null) : null;
      if (member) {
        await member.roles.remove(trial.roleId).catch(() => {});
      }
      logAction(
        '⌛',
        'Essai gratuit terminé',
        `<@${trial.userId}> — rôle **${trial.roleName}** retiré (essai expiré, lancé par ${trial.addedByTag})`,
        0x95a5a6
      );
    } catch (err) {
      console.error('❌ Erreur fin essai gratuit :', err);
      logAction('❌', 'Erreur fin essai gratuit', String(err?.message || err), 0xe74c3c);
    }

    delete state.trials[key];
  }

  if (changed) {
    saveState(state);
    await updateTrialEmbed();
  }
}
...
client.on('messageCreate', async (message) => {
  ...
  if (command === 'setuproles') {
    ...
    if (!isAdmin) {
      logAction('🚫', 'Commande refusée', `\`!setuproles\` tenté par ${message.author} (**${message.author.tag}**, pas admin)`, 0xe74c3c);
      return message.reply('❌ Seuls les administrateurs peuvent faire ça.');
    }

    const result = await postRoleReactionMessage(message.author.tag, `${message.author}`);
  ...
  if (command === 'status') {
    ...
    logAction('💬', 'Commande utilisée', `\`!status\` par ${message.author} (**${message.author.tag}**)`);
  }

  if (command === 'annonce') {
    const isAdmin = message.member?.permissions.has(PermissionsBitField.Flags.Administrator);
    if (!isAdmin) {
      logAction('🚫', 'Commande refusée', `\`!annonce\` tenté par ${message.author} (**${message.author.tag}**, pas admin)`, 0xe74c3c);
    ...
    await postAnnouncement(type, text, `Manuelle — ${message.author} (${message.author.tag})`);
  ...
  if (command === 'giveaway') {
    const isAdmin = message.member?.permissions.has(PermissionsBitField.Flags.Administrator);
    if (!isAdmin) {
      logAction('🚫', 'Commande refusée', `\`!giveaway\` tenté par ${message.author} (**${message.author.tag}**, pas admin)`, 0xe74c3c);
  ...
  if (command === 'trial') {
    const isAdmin = message.member?.permissions.has(PermissionsBitField.Flags.Administrator);
    if (!isAdmin) {
      logAction('🚫', 'Commande refusée', `\`!trial\` tenté par ${message.author} (**${message.author.tag}**, pas admin)`, 0xe74c3c);
      return message.reply('❌ Seuls les administrateurs peuvent lancer un essai gratuit.');
    }

    const targetMember = message.mentions.members?.first();
    const targetRole = message.mentions.roles?.first();
    const durationStr = args.find((a) => !a.startsWith('<@'));
    const durationMs = durationStr ? parseDuration(durationStr) : null;

    if (!targetMember || !targetRole || !durationMs) {
      return message.reply(
        `Utilisation : \`${PREFIX}trial @membre @rôle <durée ex: 30j, 24h, 2j12h>\`\n` +
        `Exemple : \`${PREFIX}trial @Pseudo @Supporter 30j\``
      );
    }
    ...
    try {
      ...
      console.error('❌ Erreur attribution rôle trial :', err);
      return message.reply("❌ Impossible d'attribuer ce rôle (vérifie la hiérarchie des rôles du bot).");
    }

    const key = `${targetMember.id}-${targetRole.id}`;
    const isRenewal = !!state.trials[key];
    const endsAt = Date.now() + durationMs;
    ...
    logAction(
      '🧪',
      isRenewal ? 'Essai gratuit renouvelé' : 'Essai gratuit démarré',
      `${targetMember} a reçu **${targetRole.name}** pendant \`${durationStr}\`\n` +
      `Expire <t:${Math.floor(endsAt / 1000)}:R>\nPar : ${message.author} (**${message.author.tag}**)`,
      0xE8B4D0
    );

    message.reply(
      `✅ Essai gratuit ${isRenewal ? 'renouvelé' : 'lancé'} pour ${targetMember} — rôle **${targetRole.name}**.\n` +
      `⏳ Expire <t:${Math.floor(endsAt / 1000)}:R>. Tableau de suivi : <#${TRIAL_CHANNEL_ID}>.`
    );
  }

  if (command === 'trialend') {
    const isAdmin = message.member?.permissions.has(PermissionsBitField.Flags.Administrator);
    if (!isAdmin) return message.reply('❌ Seuls les administrateurs peuvent arrêter un essai gratuit.');

    const targetMember = message.mentions.members?.first();
    const targetRole = message.mentions.roles?.first();

    if (!targetMember || !targetRole) {
      return message.reply(`Utilisation : \`${PREFIX}trialend @membre @rôle\``);
    }

    const key = `${targetMember.id}-${targetRole.id}`;
    const trial = state.trials[key];
    if (!trial) return message.reply('⚠️ Aucun essai actif trouvé pour ce membre et ce rôle.');
    ...
    logAction(
      '🛑',
      'Essai gratuit arrêté manuellement',
      `${targetMember} — rôle **${targetRole.name}** retiré par ${message.author} (**${message.author.tag}**)`,
      0xe74c3c
    );
    message.reply('✅ Essai gratuit terminé manuellement.');
  }
});
...
client.once('ready', async () => {
  ...
  // Essais gratuits — retrouve/rafraîchit le tableau, retire les essais déjà expirés
  // pendant que le bot était hors ligne, puis vérifie en continu
  await updateTrialEmbed();
  await checkTrials();
  setInterval(checkTrials, TRIAL_CHECK_INTERVAL_MS);
});
