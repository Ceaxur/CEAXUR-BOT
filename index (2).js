...
function buildStatusEmbed(current) {
  ...
  const embed = new EmbedBuilder()
    .setTitle(`📡 Statut de ${WEBSITE_URL.replace('https://', '').replace('http://', '')}`)
    .setColor(current.online ? 0x2ecc71 : 0xe74c3c)
    .addFields(
      { name: 'Statut', value: current.online ? '🟢 En ligne' : '🔴 Hors ligne', inline: true },
      { name: 'Temps de réponse', value: current.responseTime !== null ? `${current.responseTime} ms` : 'N/A', inline: true },
      { name: "Disponibilité aujourd'hui", value: `${uptime}%`, inline: true },
    )
...
async function sendStatusChangeAlert(current) {
  ...
  const embed = new EmbedBuilder()
    .setColor(current.online ? 0x2ecc71 : 0xe74c3c)
    .setTitle(current.online ? '🟢 Le site est de nouveau en ligne' : '🔴 Le site est hors ligne')
    .setDescription(WEBSITE_URL)
    .setTimestamp();
